...
import { permissionsService, UpdatePermissionData, Permission } from "@/services/permissionsService";
...
const EditPermissionModal = ({ 
  ...
  const [formData, setFormData] = useState<UpdatePermissionData>({
    nombre: "",
    categoria: "",
    descripcion: "",
    tipo: "lectura"
  ...
  // Cargar datos del permiso y opciones al abrir el modal
  useEffect(() => {
    if (open && permission) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await permissionsService.updatePermission(permission.id, formData);

      if (response.success) {
        toast({
          title: "Permiso actualizado",
          description: `Permiso "${formData.nombre}" actualizado exitosamente`,
        });
  ...
  const handleInputChange = (field: keyof UpdatePermissionData, value: string) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Modifica la información del permiso "{permission.nombre}".
          </DialogDescription>
        </DialogHeader>
